...
def run_cnn():
    ...
    x_shaped = tf.reshape(x, [-1, 56, 56, 1])
    ...
    with tf.Session() as sess:
        ...
        eval_images = np.reshape(eval_images, [-1,3136])
...
def create_new_conv_layer(input_data, num_input_channels, num_filters, filter_shape, pool_shape, name,init_w,init_b):
    ...
    weights = tf.Variable(init_w, name=name+'_W',dtype=tf.float32)
    bias = tf.Variable(init_b, name=name+'_b', dtype=tf.float32)
